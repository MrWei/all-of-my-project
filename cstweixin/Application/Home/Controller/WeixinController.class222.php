...
class WeixinController extends HomeController {
	...
	protected function get_keywords_data( $keywords , $openid='' ){
		...
		elseif( strpos($keywords, '@')===0 && substr_count($keywords, '@')==2 )
		{
		...
		//修改客户资料
		elseif(strpos($keywords, '#')==0 && substr_count($keywords, '#')==1)
		{
			$keywords = substr($keywords, 1);
			if( substr_count($keywords, '%')==7 ){
				...
				foreach ( $info  as $v ){
					$data['content'] .= 'NO.'.$v['id'] .' '. $v['realname'] . "\n";
				}
// 				姓名%电话%邮箱%公司%部门%职位%标签1&标签2..
// 				标签用"&"分开，最多10个,用户无标签请写"N"
				$data['content'] .= "请回复修改信息,\n(如#姓名%电话%邮箱%公司%部门%职位%标签1&标签2..%1)\n(1是编号,必填,标签用'!'分开，最多10个),确认请回复Y获取修改模版";
			}
			$data['type'] = 'text';
		}
		else{
	...
	protected function get_customer( $keywords , $openid, $type=1){
		...
		if( $userbase['gid'] == 2 ){
			switch ( $type ){
				case 1:
					$tips =  M('weixin_keywords')->where(array('title'=>'@'))->getField('content');
					//截取字符串得到姓名
					$data['content'] = $this->checkCustomer(substr($keywords, 1), $tips);
				break;
				case 2:
					//截取字符串得到姓名
		...
		if( $userbase['gid'] ==2 ){
			...
			if( (substr_count($info[6], '!')< 10)  ){
				if( $info[7] && $info[3]){
					...
					var_dump($customer);
					$temp['realname'] = $info[0]== '姓名' ? $customer['realname'] : $info[0] ; 	//客户真实姓名
					$temp['mobile']	  = $info[1]== '电话' ? $customer['mobile']   : $info[1] ;		//客户手机号
					$temp['email'] 	  = $info[2]== '邮箱' ? $customer['mobile']   : $info[2] ;		//客户邮箱
					//公司
					if( $info[3]== '公司' ){														
						$temp['companyid'] = $customer['companyid'];
					}else{
					...
					//部门
					if( $info[4] == '部门' ){																
						$temp['departmentid'] = $customer['departmentid'];
					}else{
					...
					$temp['job'] 	= $info[5]== '职位' ? $customer['job']   : $info[5] ;			//职位
					$temp['id'] = $info[7];
				}
				else{
					//组合用户信息
...
// 		$keywords = "#李四%18888888888%N%N%N%N%N%8";
	 ...
	 * @param 用户的微信 $openid
	 */
	protected function save_user($openid){
		$res = M('weixin_users')->where(array('openid'=>$openid))->find();
		if( empty($res) ){
			$data['openid'] = $openid;
			$data['create_time'] = NOW_TIME;
