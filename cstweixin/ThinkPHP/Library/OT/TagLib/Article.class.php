...
class Article extends TagLib{
	...
	protected $tags   =  array(
		...
		'artlist'     => array('attr' => 'name,category,child,page,row,field', 'close' => 1), //获取指定分类列表
	);
	...
	public function _list($tag, $content){
		...
		$parse  = '<?php ';
		$parse .= '$category=D(\'Category\')->getChildrenId('.$cate.'); var_dump( $category);';
		$parse .= '$__LIST__ = D(\'Document\')->page(!empty($_GET[C(\'VAR_PAGE\')])?$_GET[C(\'VAR_PAGE\')]:1,'.$row.')->lists(';
	...
	public function _partlist($tag, $content){
		...
		$parse  = '<?php ';
		$parse .= '$__PARTLIST__ = D(\'Document\')->part(' . $id . ',  !empty($_GET[C(\'VAR_PAGE\')])?$_GET[C(\'VAR_PAGE\')]:1, \'' . $field . '\','. $listrow .');';
		$parse .= ' ?>';
		$parse .= '<?php $page=(!empty($_GET[C(\'VAR_PAGE\')])?$_GET[C(\'VAR_PAGE\')]:1)-1; ?>';
